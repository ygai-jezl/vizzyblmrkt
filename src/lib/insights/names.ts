 ...
 * phase 4): a launch's welcome journey (journey_{launch}), its invites
 * (invite_{launch}) and lifecycle journeys (lcj_…).
 */
export async function emailNames(ctx: TenantContext, journeyIds: string[], db?: FirestoreLike): Promise<Map<string, string>> {
  ...
  const [launches, journeys] = await Promise.all([
    Promise.all([...launchIds].map((id) => repos.campaigns.getById(id).catch(() => null))),
    Promise.all(lifecycleIds.map((id) => repos.lifecycleJourneys.getById(id).catch(() => null))),
  ]);
  ...
  for (const j of journeys) if (j) out.set(j.id, j.name);
  return out;
}
